...
# Class for analyzing and (attempting) to
# future prices
# Contains a number of visualizations and analysis methods
class Stocker():

    # Initialization requires a ticker symbol
    def __init__(self, ticker, startdate='2015-01-01', enddate=None, exchange='WIKI'):
        try:
            if enddate == None:
                enddate = datetime.strftime(datetime.now() - timedelta(0), "%Y-%m-%d")
                print(f'end date is none now it is {enddate}')
            ...
            stock = web.DataReader(stock_name, data_source='yahoo', start=startdate, end=enddate)
            ...
            years_n = relativedelta(max(stock['Date']), min(stock['Date'])).years
            if years_n < self.training_years:
                self.training_years = years_n
                # print(f"number of years {years_n}")
                # raise Exception
    ...
    def plot_stock(self, start_date=None, end_date=None, stats=['Adj. Close'], plot_type='basic', save=False):
        ...
        for i, stat in enumerate(stats):
            ...
            if plot_type == 'pct':
                ...
                plt.style.use('fivethirtyeight');
                plt.plot(stock_plot['Date'], stock_plot[stat], color=colors[i], linewidth=1.5, label=stat, alpha=0.8)
                plt.xlabel('Date');
                plt.ylabel('US $');
                plt.title('%s Stock History' % self.symbol);
                plt.legend(prop={'size': 10})
                plt.grid(color='k', alpha=0.4);
        if save:
            file_name = f"./pics/Basic{self.symbol}Stock{datetime.now()}.png"
    ...
    def create_prophet_model(self, days=0, resample=False, save=False):
        ...
        plt.title(title)
        if save:
            file_name = f"./pics/{self.symbol}StockModel{datetime.now()}.png"
            plt.savefig(file_name)
            return model, future, file_name
        plt.show()
    ...
    def predict_future(self, days=30, save=False):
        ...
        # Plot the estimates
        ax.plot(future_increase['Date'], future_increase['estimate'], 'g^', ms=8, label='Pred. Increase')
        ax.plot(future_decrease['Date'], future_decrease['estimate'], 'rv', ms=8, label='Pred. Decrease')

        # Plot errorbars
        ax.errorbar(future['Date'].dt.to_pydatetime(), future['estimate'],
                    yerr=future['upper'] - future['lower'],
                    capthick=1.4, color='c', linewidth=2,
                    ecolor='black', capsize=2, elinewidth=0.5, label='Pred with Range')
        ...
        if save:
            file_name = f"./pics/Prediction{self.symbol}Stock{datetime.now()}.png"
            plt.savefig(file_name)
            return file_name
        plt.show()

    def changepoint_prior_validation(self, start_date=None, end_date=None,
                                     changepoint_priors=[0.15,0.3,0.5, 0.6]):

        # Default start date is two years before end of data
        # Default end date is one year before end of data
        # CHANGE: start date from two years to one and end date from one to none
        if start_date is None:
            start_date = self.max_date - pd.DateOffset(years=1)
        if end_date is None:
            end_date = self.max_date
        ...
        # plt.legend(prop={'size':10})
        # plt.show();
        return results.sort_values("test_err").iloc[0].cps
